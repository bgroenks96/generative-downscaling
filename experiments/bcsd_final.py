...
def fit_bcsd(lr_train, lr_test, lr_obs_train, lr_obs_test, hr_train, hr_test):
    ...
    metrics = base.eval_metrics(indices, y_true, y_pred, hr_test.coords)
    ...
    fig = base.plot_error_maps(metrics, hr_test.lat, hr_test.lon)
...
def bcsd(scale, region, var, auth, **kwargs):
    ...
    data_lo, data_hi = load_data('erai/daily-1deg', data_hr, region, auth, scale=scale)
    ...
    lr_obs_train = data_obs_lo.isel(Time=slice(0,data_lo.Time.size-365))
    lr_obs_test = data_obs_lo.isel(Time=slice(data_lo.Time.size-365, data_lo.Time.size+1))
    ...
    fit_bcsd(lr_train, lr_test, lr_obs_train, lr_obs_test, hr_train, hr_test)